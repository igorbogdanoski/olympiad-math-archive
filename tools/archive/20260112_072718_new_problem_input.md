...
class SolutionScene(Scene):
    def construct(self):
        ...
        triangle_group = VGroup(tri, sq_a, lbl_a, lbl_b, lbl_k)
        triangle_group.move_to(UP*1.5)
        
        # Formulas
        eq1 = MathTex("a^2 + b^2 = k^2", color=BLACK)
        eq2 = MathTex("k + a = m^2 + n^2 + m^2 - n^2", color=BLUE).next_to(eq1, DOWN)
        eq3 = MathTex("k + a = 2m^2 = m^2 + m^2", color=RED).next_to(eq2, DOWN)
        formulas_group = VGroup(eq1, eq2, eq3).arrange(DOWN, buff=0.4).move_to(DOWN*1.5)
        
        # Add to scene
        self.add(triangle_group, formulas_group)
```