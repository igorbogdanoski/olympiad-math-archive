...
class SolutionScene(Scene):
    def construct(self):
        ...
        dots = VGroup(*[Dot(axes.c2p(i, 0), radius=0.1).set_color(BLUE) for i in range(0, 6)])
        ...
        ineq_a = MathTex("0 < \\frac{a}{9} \\leq 1").set_color(BLACK).to_edge(UP).shift(LEFT * 2)
        ineq_b = MathTex("3 < \\frac{b}{8} \\leq 4").set_color(BLACK).to_edge(UP).shift(RIGHT * 2)
        ...
        self.add(axes, number_line, dots, forbidden, ineq_a, ineq_b, bracket, parenthesis, interval_line, calc)
