...
class SolutionScene(Scene):
  def construct(self):
    ...
    # --- TITLE ---
    title = MathTex(r"(x+y)(y+z)(z+x) = 1680", color=BLACK).to_edge(UP)
    subtitle = MathTex(r"xy + yz + zx = 96", color=BLACK).next_to(title, DOWN)
    # --- IDENTITY ---
    identity = MathTex(
      r"(x+y+z)(xy+yz+zx) - xyz = 1680", 
      color=BLUE
    ).shift(UP * 0.5)
    # --- SOLUTION ---
    logic = VGroup(
      MathTex(r"3x^2 \le 96 \implies x \le 5", color=BLACK),
      MathTex(r"\text{За } x=3 \implies y+z=16, yz=48", color=BLACK),
      MathTex(r"(x, y, z) = (3, 4, 12)", color=BLACK)
    ).arrange(DOWN, buff=0.5).next_to(identity, DOWN, buff=0.8)
    # --- FINAL CALCULATION ---
    final_box = MathTex(
      r"100(3) + 10(4) + 12 = 352", 
      color=RED
    ).to_edge(DOWN).shift(UP * 0.5)
    rect = SurroundingRectangle(final_box, color=RED, stroke_width=2)
    self.add(title, subtitle, identity, logic, final_box, rect)
```