...
<details>
<summary>Чекор 1: Анализа на основниот случај ($n=1$)</summary>
...
class SolutionScene(Scene):
    def construct(self):
        ...
        # --- CONFIGURATION ---
        # Definiranje na broevite
        text_19 = MathTex("19", color=BLACK).shift(LEFT * 1.5 + UP * 0.5)
        text_69 = MathTex("69", color=BLACK).shift(RIGHT * 1.5 + UP * 0.5)
        plus = MathTex("+", color=BLACK).move_to(ORIGIN + UP * 0.5)
        
        # Sobiranje
        line = Line(LEFT * 2, RIGHT * 2, color=BLACK, stroke_width=4).next_to(plus, DOWN)
        result = MathTex("88", color=RED).next_to(line, DOWN)
        
        # Delivost
        div_logic = MathTex("88 = 2 \\times 44", color=BLACK).next_to(result, DOWN, buff=0.8)
        conclusion = MathTex("44 \\mid 88 \\implies 44 \\mid (19^n + 69^n)", color=BLUE).to_edge(DOWN)
        
        # Za n neporen (LaTeX label)
        n_cond = MathTex("n \\in \{1, 3, 5, \\dots\}", color=BLACK).to_edge(UP)

        # Dodavanje na objektite
        self.add(n_cond, text_19, plus, text_69, line, result, div_logic, conclusion)
```