...
class SolutionScene(Scene):
    def construct(self):
        ...
        # Points on Circle (choose three well-separated points)
        phi_A = 110 * DEGREES
        phi_B = 230 * DEGREES  # 120° apart from A
        phi_C = 350 * DEGREES  # 120° apart from B
        
        A = R * np.array([np.cos(phi_A), np.sin(phi_A), 0])
        B = R * np.array([np.cos(phi_B), np.sin(phi_B), 0])
        C = R * np.array([np.cos(phi_C), np.sin(phi_C), 0])
        
        # Re-calculating alpha for the current points
        vec_AB = B - A
        vec_AC = C - A
        denom = np.linalg.norm(vec_AB) * np.linalg.norm(vec_AC)
        if denom < 1e-8:
          ang_A = 1.0  # fallback to avoid division by zero
        else:
          ang_A = np.arccos(np.dot(vec_AB, vec_AC) / denom)
        ...
        # Avoid division by zero in sin(ang_A)
        eps = 1e-8
        sin_ang_A = np.sin(ang_A) if abs(np.sin(ang_A)) > eps else eps
        M = A + (vec_AB / (len_AB + eps)) * (len_AC / sin_ang_A)
        N = A + (vec_AC / (len_AC + eps)) * (len_AB / sin_ang_A)
