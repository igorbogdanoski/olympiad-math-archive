...
class SolutionScene(Scene):
    def construct(self):
        ...
        # --- CONFIGURATION ---
        # Definiranje na tekstot
        cond = MathTex("xyz = 2024(x + y + z)", color=BLACK).to_edge(UP)
        subst = MathTex("2024 = \\frac{xyz}{x + y + z}", color=BLUE).next_to(cond, DOWN)
        
        term_l = MathTex("\\frac{2024}{x} + y", "=", "\\frac{y(x + y + 2z)}{x + y + z}", color=BLACK)
        term_r = MathTex("\\frac{2024}{y} + x", "=", "\\frac{x(x + y + 2z)}{x + y + z}", color=BLACK)
        
        # Pozicioniranje
        group = VGroup(term_l, term_r).arrange(DOWN, buff=0.8).center()
        
        # Logic link
        arrow = DoubleArrow(term_l.get_right(), term_r.get_right(), color=RED, stroke_width=4)
        ratio = MathTex("\\frac{LHS_{term}}{y} = \\frac{RHS_{term}}{x}", color=BLACK).next_to(arrow, RIGHT)

        # Final setup
        self.add(cond, subst, group, arrow, ratio)
        
        # Prikaz na resenieto pod b
        sol_b = MathTex("(x, y, z) = (46, 46, 2024)", color=BLACK).to_edge(DOWN).shift(UP*0.5)
