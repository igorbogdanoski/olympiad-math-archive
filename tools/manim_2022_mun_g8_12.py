...
class Problem_2022_mun_g8_12(Scene):
    def construct(self):
        ...
        # Labels
        lbl_width = MathTex("4 \text{ m}", color=BLACK).next_to(terrace, UP, buff=0.1)
        lbl_height = MathTex("3 \text{ m}", color=BLACK).next_to(terrace, RIGHT, buff=0.1)
        
        # Highlight one tile
        # Bottom left tile
        tile = Rectangle(width=6/10, height=4.5/10, color=RED, fill_opacity=0.5).move_to(np.array([-3 + 3/10, -2.25 + 2.25/10, 0]))
        
        lbl_tile = MathTex("40 \text{ cm} \\times 30 \text{ cm}", color=RED, font_size=24).next_to(tile, UR, buff=0.1)
        ...
        calc2 = MathTex(r"\text{Area Tile} = 40 \times 30 \text{ cm}^2", color=BLACK, font_size=30).next_to(calc1, DOWN, aligned_edge=LEFT)
        calc3 = MathTex(r"\text{Num Tiles} = \frac{400}{40} \times \frac{300}{30} = 10 \times 10 = 100", color=BLACK, font_size=30).next_to(calc2, DOWN, aligned_edge=LEFT)
