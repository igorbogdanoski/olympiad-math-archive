...
for name, path in files_to_check.items():
    ...
    # Extract problem_id
    problem_id_match = re.search(r"problem_id:\s*(.+)", content)
    problem_id = problem_id_match.group(1).strip() if problem_id_match else None
    
    has_image_tag = "![" in content
    
    # Check if image exists in assets using problem_id
    found_image = None
    if problem_id:
        for ext in image_extensions:
            image_name = problem_id + ext
            image_path = os.path.join(assets_dir, image_name)
            if os.path.exists(image_path):
                found_image = image_name
                break
    
    # Also check using the filename if problem_id didn't work
    if not found_image:
         for ext in image_extensions:
            image_name = name + ext
            image_path = os.path.join(assets_dir, image_name)
    ...
    print(f"{name} (ID: {problem_id}): {', '.join(status)}")
