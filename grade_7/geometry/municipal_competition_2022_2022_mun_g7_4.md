---
grade: 7
field: geometry
difficulty: 3
problem_type: calculation
source: "Municipal_Competition_2022"
problem_id: 2022_mun_g7_4
language_original: <mk | en | sr | hr | ...>
translated: false

# --- GEOMETRY SKILLS ---
geometry_style: synthetic
primary_skill: visual_reasoning
related_skills:
  - logic
prerequisites:
  - volume_formula
  - unit_conversion

# --- VISUALIZATION ---
visual_prompt: "No visual prompt provided."

tags:
  - geometry
  - olympiad
  - volume
  - cube
  - units
---

[⬅️ Назад кон Индексот](../../README.md) | [🧰 Skill: visual_reasoning](../../../tools/skill_guides/visual_reasoning.md)

# Коцки во коцка

## 📝 Текст на задачата
Колку најмногу коцки со волумен $8 \text{ mm}^3$ можат да се сместат во внатрешноста на коцка со раб $4 \text{ cm}$?

## 📐 Скица

![Визуелизација](../../assets/images/2022_mun_g7_4.png)


> **👨‍💻 Geo-Mentor Code:**
> Одете во `assets/manim_code_log.md`, копирајте го кодот за `Task_2022_mun_g7_4` и генерирајте ја сликата.



> **👨‍💻 Geo-Mentor Code:**
> Одете во `assets/manim_code_log.md`, копирајте го кодот за `Task_2022_mun_g7_4` и генерирајте ја сликата.

## 🧠 Анализа
**Зошто е оваа задача тешка?**
Прво најдете ја страната на малата коцка ($a=\sqrt[3]{8}$). Потоа претворете го работ на големата коцка во истата мерна единица (mm).

**Конструктивен потег:**
Прво најдете ја страната на малата коцка ($a=\sqrt[3]{8}$). Потоа претворете го работ на големата коцка во истата мерна единица (mm).

## 💡 Решение

<details>
<summary>👀 Прикажи го решението</summary>

**Чекор 1: Страна на малата коцка**
$V = a^3 = 8 \implies a = 2 \text{ mm}$.

**Чекор 2: Страна на големата коцка**
$A = 4 \text{ cm} = 40 \text{ mm}$.

**Чекор 3: Број на коцки**
По должина, ширина и висина собира:
$$ n = \frac{40}{2} = 20 \text{ коцки} $$
Вкупно: $N = 20^3 = 8000$.

</details>


## 🏁 Заклучок
<Краен резултат.>

## 👩‍🏫 За наставници
Важно е да се провери дали малите коцки „совршено“ ја пополнуваат големата (дали 40 е деливо со 2). Во овој случај да.