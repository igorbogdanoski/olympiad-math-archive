---
grade: 10
field: geometry
difficulty: 2
problem_type: calculation
source: "Municipal_Competition_2022"
problem_id: 2022_mun_y2_11b
language_original: <mk | en | sr | hr | ...>
translated: false

# --- GEOMETRY SKILLS ---
geometry_style: synthetic
primary_skill: trigonometry
related_skills:
  - logic
prerequisites:
  - pythagorean_theorem
  - trig_definitions

# --- VISUALIZATION ---
visual_prompt: "Draw a right-angled triangle ABC with C=90 degrees. AC is the base, length 2. BC is the height. Angle A is alpha. Label AC=2 and tan(alpha)=3/4."

tags:
  - geometry
  - olympiad
  - right_triangle
  - trigonometry
---

[⬅️ Назад кон Индексот](../../README.md) | [🧰 Skill: trigonometry](../../../tools/skill_guides/trigonometry.md)

# Периметар на правоаголен триаголник

## 📝 Текст на задачата
Најди го периметарот на правоаголниот триаголник $ABC$ ($\angle C = 90^\circ$), ако се знае дека катетата $AC = 2$ и $\tan \alpha = \frac{3}{4}$.

## 📐 Скица

![Визуелизација](../../assets/images/2022_mun_y2_11b.png)


> **👨‍💻 Geo-Mentor Code:**
> Одете во `assets/manim_code_log.md`, копирајте го кодот за `Task_2022_mun_y2_11b` и генерирајте ја сликата.



> **👨‍💻 Geo-Mentor Code:**
> Одете во `assets/manim_code_log.md`, копирајте го кодот за `Task_2022_mun_y2_11b` и генерирајте ја сликата.

## 🧠 Анализа
**Зошто е оваа задача тешка?**
Искористи ја дефиницијата за тангенс: $\tan \alpha = \frac{a}{b}$. Бидејќи $b=AC=2$, можеш да ја најдеш катетата $a$. Потоа најди ја хипотенузата со Питагора.

**Конструктивен потег:**
Искористи ја дефиницијата за тангенс: $\tan \alpha = \frac{a}{b}$. Бидејќи $b=AC=2$, можеш да ја најдеш катетата $a$. Потоа најди ја хипотенузата со Питагора.

## 💡 Решение

<details>
<summary>👀 Прикажи го решението</summary>

**Чекор 1: Наоѓање на катетата $a$ ($BC$)**
Во правоаголен триаголник:
$$ \tan \alpha = \frac{a}{b} = \frac{BC}{AC} $$
Дадено е $\tan \alpha = \frac{3}{4}$ и $AC = 2$.
$$ \frac{a}{2} = \frac{3}{4} \implies a = \frac{3 \cdot 2}{4} = 1,5 $$

**Чекор 2: Наоѓање на хипотенузата $c$ ($AB$)**
Според Питагорова теорема:
$$ c^2 = a^2 + b^2 = (1,5)^2 + 2^2 $$
$$ c^2 = 2,25 + 4 = 6,25 $$
$$ c = \sqrt{6,25} = 2,5 $$

**Чекор 3: Периметар**
$$ L = a + b + c = 1,5 + 2 + 2,5 = 6 $$

**Одговор:** 6.

</details>


## 🏁 Заклучок
<Краен резултат.>

## 👩‍🏫 За наставници
Ова е триаголник сличен на (3, 4, 5), скалиран со фактор 0.5.