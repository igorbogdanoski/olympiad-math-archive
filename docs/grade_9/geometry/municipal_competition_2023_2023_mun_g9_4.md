---
grade: 9
field: geometry
difficulty: 4
problem_type: calculation
source: "Municipal_Competition_2023"
problem_id: 2023_mun_g9_4
language_original: <mk | en | sr | hr | ...>
translated: false

# --- GEOMETRY SKILLS ---
geometry_style: synthetic
primary_skill: algebraic_manipulation
related_skills:
  - logic
prerequisites:
  - pythagorean_theorem

# --- VISUALIZATION ---
visual_prompt: "No visual prompt provided."

tags:
  - geometry
  - olympiad
  - quadrilateral
  - area
  - pythagoras
  - algebraic_geometry
---

[⬅️ Назад кон Индексот](../../README.md) | [🧰 Skill: algebraic_manipulation](../../../tools/skill_guides/algebraic_manipulation.md)

# Плоштина на четириаголник

## 📝 Текст на задачата
Даден е четириаголник $ABCD$ така што збирот на страните $AB$ и $AD$ е 10 cm, а аглите $\angle A$ и $\angle C$ се прави ($90^\circ$). Ако $BC = CD$, пресметај ја плоштината на четириаголникот.

## 📐 Скица

![Визуелизација](../../assets/images/2023_mun_g9_4.png)


> **👨‍💻 Geo-Mentor Code:**
> Одете во `assets/manim_code_log.md`, копирајте го кодот за `Task_2023_mun_g9_4` и генерирајте ја сликата.



> **👨‍💻 Geo-Mentor Code:**
> Одете во `assets/manim_code_log.md`, копирајте го кодот за `Task_2023_mun_g9_4` и генерирајте ја сликата.

## 🧠 Анализа
**Зошто е оваа задача тешка?**
Повлечете ја дијагоналата $BD$. Таа ја дели фигурата на два правоаголни триаголници. Изразете ја хипотенузата $BD^2$ на два начина. Ова ќе ви даде врска меѓу страните. Плоштината е збир на плоштините на двата триаголници.

**Конструктивен потег:**
Повлечете ја дијагоналата $BD$. Таа ја дели фигурата на два правоаголни триаголници. Изразете ја хипотенузата $BD^2$ на два начина. Ова ќе ви даде врска меѓу страните. Плоштината е збир на плоштините на двата триаголници.

## 💡 Решение

<details>
<summary>👀 Прикажи го решението</summary>

**Чекор 1: Ознаки**
Нека $AB=a, AD=b$. Дадено е $a+b=10$.
Нека $BC=CD=c$. Дијагоналата е $BD=d$.

**Чекор 2: Питагорова теорема**
Од $\triangle ABD$ (прав агол кај A): $d^2 = a^2 + b^2$.
Од $\triangle BCD$ (прав агол кај C): $d^2 = c^2 + c^2 = 2c^2$.
Значи: $2c^2 = a^2 + b^2 \implies c^2 = \frac{a^2+b^2}{2}$.

**Чекор 3: Плоштина**
$$ P = P_{ABD} + P_{BCD} = \frac{ab}{2} + \frac{c^2}{2} $$
Заменуваме $c^2$:
$$ P = \frac{ab}{2} + \frac{1}{2} \left( \frac{a^2+b^2}{2} \right) = \frac{2ab + a^2 + b^2}{4} $$

**Чекор 4: Препознавање на полн квадрат**
Броителот е $(a+b)^2$!
$$ P = \frac{(a+b)^2}{4} $$

**Чекор 5: Пресметка**
Дадено е $a+b=10$.
$$ P = \frac{10^2}{4} = \frac{100}{4} = 25 \text{ cm}^2 $$

</details>


## 🏁 Заклучок
<Краен резултат.>

## 👩‍🏫 За наставници
Ова е истата задача како во претходниот сет (2024 Grade 9 Problem 4), но сега е правилно класифицирана во 2023. Решението е идентично.