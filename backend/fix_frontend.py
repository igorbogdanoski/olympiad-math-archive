...
# Точната и чиста верзија на Svelte кодот
svelte_code = """<script>
  ...
  const highSchoolGrades = [
    { label: "I Година", value: "HighSchool_I" },
    { label: "II Година", value: "HighSchool_II" },
    { label: "III Година", value: "HighSchool_III" },
    { label: "IV Година", value: "HighSchool_IV" }
  ];
  ...
  async function fetchCurriculum(grade) {
    ...
    try {
      const response = await fetch(`http://localhost:8000/curriculum/${grade}`);
      if (response.ok) {
...
<div class="max-w-6xl mx-auto p-4 font-sans text-slate-800">
  ...
  <div class="bg-white rounded-2xl shadow-sm border border-gray-100 p-6 mb-8 text-center">
    ...
    <div class="space-y-6">
      <div>
        <div class="text-xs font-bold text-gray-400 uppercase tracking-wider mb-3">Основно Образование</div>
        <div class="flex flex-wrap justify-center gap-2">
          {#each primaryGrades as grade}
            <button 
              class="w-10 h-10 md:w-12 md:h-12 rounded-full font-bold text-sm transition-all duration-200 border-2 flex items-center justify-center
              {activeGrade === grade.value 
                ? 'bg-blue-600 text-white border-blue-600 scale-110 shadow-md ring-2 ring-blue-200' 
                : 'bg-white text-gray-600 border-gray-200 hover:border-blue-300 hover:bg-blue-50'}"
              on:click={() => fetchCurriculum(grade.value)}
            >
              {grade.label}
            </button>
          {/each}
      ...
      <div>
        <div class="text-xs font-bold text-gray-400 uppercase tracking-wider mb-3">Средно Образование</div>
        <div class="flex flex-wrap justify-center gap-3">
          {#each highSchoolGrades as grade}
            <button 
  ...
  {#if loading}
    <div class="flex flex-col items-center justify-center h-64 opacity-75">
      ...
      <p class="mt-4 text-gray-500 font-medium animate-pulse">Вчитување на програмата...</p>
    </div>
  ...
  {:else}
    <div class="text-center py-12 bg-gray-50 rounded-xl border border-dashed border-gray-300">
      <p class="text-gray-500 text-lg">Нема пронајдено податоци.</p>
    </div>
  {/if}
...
# Патеката до фајлот (релативно од backend)
file_path = os.path.join("..", "web", "src", "components", "CurriculumViewer.svelte")

try:
    with open(file_path, "w", encoding="utf-8") as f:
        f.write(svelte_code)
    print(f"✅ УСПЕШНО поправен: {file_path}")
    print("Сега освежи го прелистувачот!")
except FileNotFoundError:
    print(f"❌ Грешка: Не можам да го најдам фајлот: {file_path}")