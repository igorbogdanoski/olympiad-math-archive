...
class WorksheetProblemRenderer {
  ...
  /**
   * Fetch problems from API by IDs or filters
   * @param {Array<string>} problemIds - Array of problem IDs (optional)
   * @param {Object} filters - Filter options (topic, grade, difficulty, etc)
   * @returns {Promise<Array>} Array of problem objects
   */
  async fetchProblems(problemIds = [], filters = {}) {
    try {
      ...
      if (filters.min_difficulty) params.append('min_difficulty', filters.min_difficulty);
      if (filters.max_difficulty) params.append('max_difficulty', filters.max_difficulty);
      if (filters.limit) params.append('limit', filters.limit);
      
      // Default to getting reasonable subset
      if (!params.has('limit')) params.append('limit', '500');
      if (!params.has('min_difficulty')) params.append('min_difficulty', '1');
      if (!params.has('max_difficulty')) params.append('max_difficulty', '10');
      
      const url = `${this.apiBase}/api/problems?${params.toString()}`;
      console.log('[fetchProblems] Fetching from URL:', url);
      
      const response = await fetch(url);
      
      if (!response.ok) {
        console.error('[fetchProblems] HTTP error:', response.status, response.statusText);
        throw new Error(`HTTP error! status: ${response.status}`);
      }
      
      const data = await response.json();
      console.log('[fetchProblems] Received data:', data);
      console.log('[fetchProblems] Problems array length:', (data.problems || []).length);
      return data.problems || [];
    } catch (error) {
